...
export default function schedulerExtension(pi: ExtensionAPI): void {
	...
	const _updateUI = createUpdateUI(
		() => cachedUi,
		() => engine,
	);
	...
	pi.registerCommand("loop", {
		description:
			"创建重复定时任务。用法: /loop [interval] <prompt>，如 /loop 5m check deploy",
		handler: async (args: string, ctx: ExtensionContext) => {
			...
			if (!prompt) {
				...
				if (!content) {
					ctx.ui.notify(
						"未找到 loop.md（.pi/loop.md 或 ~/.pi/agent/loop.md）。请输入 prompt 或创建 loop.md。",
						"warning",
					);
					return;
				}
			...
			ctx.ui.notify(
				`⏱ 循环任务已创建: "${prompt.slice(0, 50)}" 每 ${Math.round(intervalMs / 60_000)}m  id=${timer.id}`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("remind", {
		description:
			"创建一次性提醒。用法: /remind <interval> <message>，如 /remind 45m check tests",
		handler: async (args: string, ctx: ExtensionContext) => {
			const parsed = parseLoopArgs(args);
			if (!parsed.prompt) {
				ctx.ui.notify(
					"用法: /remind <interval> <message>，如 /remind 45m check tests",
					"warning",
				);
				return;
			}
			...
			ctx.ui.notify(
				`⏰ 提醒已创建: "${parsed.prompt.slice(0, 50)}"  ${Math.round(parsed.intervalMs / 60_000)}m 后  id=${timer.id}`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("tasks", {
		description:
			"管理定时任务。/tasks 打开交互面板，/tasks cancel <id> 直接取消",
		handler: async (args: string, ctx: ExtensionContext) => {
			const parts = args.trim().split(/\s+/);
			if (parts[0] === "cancel" && parts[1]) {
				const ok = engine.cancel(parts[1]);
				ctx.ui.notify(
					ok ? `已取消任务 ${parts[1]}` : `未找到任务 ${parts[1]}`,
					ok ? "info" : "warning",
				);
				return;
			}
	...
	pi.registerTool({
		...
		label: "Schedule",
		description:
			"定时任务管理。创建定时/重复消息、列出任务、取消任务。用户说'提醒我'、'定时'、'每X分钟'、'loop'时使用此工具。",
		...
		async execute(
			...
			if (action === "create") {
				...
				if (!p || !ms) {
					return {
						content: [
							{ type: "text", text: "缺少 prompt 或 interval_ms 参数" },
						],
					};
				}
			...
			if (action === "list") {
				...
				const lines = active.map((t) => {
					const remaining = Math.max(
						0,
						Math.round((t.expiresAt - Date.now()) / 60_000),
					);
	...
	pi.on(
		...
		async (_event: SessionStartEvent, ctx: ExtensionContext) => {
			...
			engine.cleanup();
			const newUpdateUI = createUpdateUI(
				() => cachedUi,
				() => engine,
			);
	...
	pi.on(
		...
		async (_event: BeforeAgentStartEvent, ctx: ExtensionContext) => {
			...
			restored = true;
			const entries = ctx.sessionManager.getEntries();
			const hasTimers = entries.some(
				(e: SessionEntry) =>
					e.type === "custom" &&
					(e as { customType?: string }).customType === "scheduler",
			);
			if (hasTimers) {
				engine.restore(entries);
				ctx.ui.notify(
